...
public class ButtonNodeTree extends JPanel {
    ...
    // Constructor
    public ButtonNodeTree(ActionListener actionListener, MouseListener mouseListener) {
    ...
    private void addCreature(Creature creature, Node node) {
        ...
        // Add its treasures to the tree
        for(Treasure treasure : creature.getTreasures())
            addTreasure(treasure, node);
            
        // Add its artifacts to the tree
        for(Artifact artifact : creature.getArtifacts())
            addArtifact(artifact, node);
        
        // Add its jobs to the tree
        for(Job job : creature.getJobs())
            addJob(job, node);
    }
    
    // Creates a tree structure from a Job. Handles as root
    //  or if it is a child of the root.
    private void addJob(Job job, Node node) {
        JButton button;
        ...
        button.addActionListener(actionListener);
        button.addMouseListener(mouseListener);
        button.setToolTipText(job.toString());
        node.add(button);
    }
